...
protocol WRWeekViewDataSource : class {
    func wrWeekViewMinHour(_ weekView: WRWeekView) -> Int
    func wrWeekViewMaxHour(_ weekView: WRWeekView) -> Int
}


public class WRWeekView: UIView {
    weak var datasource : WRWeekViewDataSource!
    ...
    fileprivate func getDateForY(_ y: CGFloat) -> (Int, Int) {
        ...
        let hour = Int(adjustedY / flowLayout.hourHeight) + flowLayout.minHour
        let minute = Int((adjustedY / flowLayout.hourHeight - CGFloat(hour) + CGFloat(flowLayout.minHour)) * 60)
